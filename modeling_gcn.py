...
class Cliprec(nn.Module):
    def __init__(self,clip_em,pos_A,neg_A,u_num,video_num,total_u_id_list,total_v_id_list,step_train=True):
        ...
        self.other_dim = 128
        self.pos_weight = 0.8  #原0.8
        self.neg_weight = -0.2  #原-0.2
    ...
    def forward(self, batch, train_step):
        ...
        if train_step:
            ...
            for k in range(len(num_valid_clips)):
                ...
                #print(torch.mean(w[np.where(self.total_v_id_list == video_id2)[0][0]*self.num_clips:np.where(self.total_v_id_list == video_id2)[0][0]*self.num_clips+num_valid_clips[k]-1,:],dim=0,keepdim=True).unsqueeze(0))

                #for a in range(int(num_valid_clips[k]-1)):
                #    clip_gcn_pos2.append(w[np.where(self.total_v_id_list == video_id2)[0][0]*self.num_clips+a,:].unsqueeze(0).unsqueeze(0))
                    #print(w[np.where(self.total_v_id_list == video_id2)[0][0]*self.num_clips+a,:].unsqueeze(0).size())
