...
from modeling_g import Cliprec
...
total_v_id_list = np.unique(np.array(interaction['photo_id']))  #video列表
#train = pd.read_csv('./fast_train_samples1.csv')
train = pd.read_csv('./fast_train_sample_ablation.csv')
...
dataset_train = CliprecDataset(num_frm=3,ensemble_n_clips=4,num_labels=4,datadir='./fast_train_sample.csv',videodir='./micro_video2', visual_feature_dir = './fast_video_feature')
...
# with open('fast_gcn'+time+".txt", "a") as f:
epochs = 10
best_val_auc = 0
best_recall3 = 0
best_recall5 = 0
...
for epoch in range(epochs):
    ...
    for step, batch in loop:
        ...
        train_logloss = logloss_cal(label.cpu().detach().numpy(),outputs.cpu().detach().numpy())
        av_loss += loss.item()
        loss.backward()
        optimizer.step()

        auc = roc_auc_score(label,outputs.cpu().detach().numpy())
        print('epoch:',epoch,'loss:',loss.item(),'auc:',auc,'logloss:',train_logloss)
    ...
    if epoch>-1:
        with torch.no_grad():
            ...
            for step, batch in enumerate(val_loader):
                ...
                #print(val_output.detach().numpy().shape)
            ...
            recall3 = recall3/i
            recall5 = recall5/i
            val_auc1 = roc_auc_score(p_matrix1[:,-1],p_matrix1[:,-2])
            logloss1 = logloss_cal(np.array(p_matrix1[:,-1]),np.array(p_matrix1[:,-2])+0.00005)
            if(val_auc>best_val_auc):
                #torch.save(model.state_dict(),'./model/best_auc.pth')
                best_val_auc = val_auc
            print('Test:\n')
            print("epoch:"+str(epoch)+' val_auc:'+str(val_auc1)+' Recall@3:'+str(recall3)+' Recall@5:'+str(recall5)+' NDCG@3:'+str(ndcg3)+' NDCG@5:'+str(ndcg5)+'logloss'+str(logloss1)+'\n')
